...
export default function CadastrarPetAdmin() {
  ...
  const handleSalvar = async () => {
    if (!id) {
      Alert.alert("Erro", "ID do animal não encontrado.");
      return;
    }
    if (!microchip && !procedimento) {
      Alert.alert("Aviso", "Preencha o microchip ou algum procedimento/vacina.");
      return;
    }
    ...
    try {
      ...
      if (microchip) {
        updateData.microchip = microchip;
      }

      if (procedimento) {
        updateData.vacinas = arrayUnion({
          nome: procedimento,
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.section}>
        <Text style={styles.label}>Registro de Microchip (Opcional se for só vacina)</Text>
        <TextInput 
